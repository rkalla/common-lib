...
public class CharTokenizer extends AbstractTokenizer<char[]> {

	public void setSource(char[] source, char[] delimiters)
	...
	public IToken<char[]> nextToken() throws IllegalStateException {
		...
		if (!foundAllTokens)
			token = new CharToken(tsIndex, (teIndex - tsIndex), source);
	...
	public int[] nextTokenFast() throws IllegalStateException {
		...
		if (!foundAllTokens) {
			 ...
			 * delims, we scan forward to the first non-delim char, then hunt
